...
float laser_frame_x = 0.0;
float laser_frame_y = 0.0;
float laser_frame_z = 0.0;
float laser_frame_theta = 0.0;

void tfUpdate(const std_msgs::Float32MultiArray::ConstPtr& msg){
	
	//Send updated TF transform
	static tf2_ros::TransformBroadcaster br;
	geometry_msgs::TransformStamped odomTransform;
	geometry_msgs::TransformStamped laserTransform;
	tf2::Quaternion q;
	
	//dynamic odometry update
	odomTransform.header.stamp = ros::Time::now();
	odomTransform.header.frame_id = "odom";
	odomTransform.child_frame_id = "base_link";
	odomTransform.transform.translation.x = msg->data[0];
	odomTransform.transform.translation.y = msg->data[1];
	odomTransform.transform.translation.z = 0.0;
	q.setRPY(0, 0, msg->data[2]);
	...
	laserTransform.transform.translation.x = laser_frame_x;
	laserTransform.transform.translation.y = laser_frame_y;
	laserTransform.transform.translation.z = laser_frame_z;
	q.setRPY(0, 0, laser_frame_theta);
	...
	br.sendTransform(laserTransform); 
}
...
int main(int argc, char * argv[]) {
    ...
    ros::Subscriber sub = nh.subscribe<std_msgs::Float32MultiArray>("/pose_estimate", 1000, tfUpdate);
	...
	ros::Rate loop_rate(LOOPRATE); //10Hz
